...
setwd("~/GSE260589_DGE")
getwd()
counts_path <- "~/GSE260589_DGE/DGE_output/post-outlier_removal_2/GSE260389_differential_expression.csv"
meta_path   <- "~/GSE260589_DGE/DGE_output/post-outlier_removal_2/GSE260389_metadata_modified_post_outliers_removal.csv"
out_dir_p2  <- "~/GSE260589_DGE/WGNCA_GSE260589"   # Phase 2 outputs
ws_fig <- file.path(out_dir_p2, "figures")

project_name <- "GSE260589"
...
normcount <- read.csv(counts_path, header = TRUE, sep = ",", check.names = FALSE)
temp.norm <- normcount %>%
  filter(!is.na(Gene_symbol), Gene_symbol != "")
# temp.norm <- temp.norm[, -c(1, 3:11)]

rowGroup <- as.character(temp.norm$Gene_symbol)
rownames(temp.norm) <- make.unique(temp.norm$Gene_symbol)
temp.norm <- temp.norm[, -c(1, 251:1813)]
...
max(temp.norm)
temp.norm <- as.matrix(temp.norm)
temp.norm <- log2(temp.norm+1)
max(temp.norm)
...
###############################################################################
## Load metadata
GSE260589.meta <- read.csv(meta_path, header = TRUE, sep = ",", check.names = FALSE)
temp.meta <- GSE260589.meta
rownames(temp.meta) <- temp.meta[, 1]
temp.meta <- temp.meta[, -1]
all.equal(colnames(temp.norm), rownames(temp.meta))
...
par(mar = c(7, 5, 3, 2))
plot(sampleTree, main = "Sample Dendrogram (TempOSeq-LOG2T) - GSE260589 Outlier Check",
...
###############################################################################
# PHASE 2: WGCNA on log2-norm data
###############################################################################
out_dir <- out_dir_p2
fig_dir <- ws_fig
...
saveRDS(datExpr, file.path(out_dir, "GSE260589_datExpr.rds"))
logit(sprintf("  Saved: GSE260589_datExpr.rds (%d samples x %d genes)",
...
all.equal(rownames(datExpr), rownames(meta_final))

# Note: diagnosis uses Unicode right single quote (U+2019)
is_male <- meta_final$Sex == "male"
is_female <- meta_final$Sex == "female"
is_control_incubator <- meta_final$Dose_condition == "None_incubator_control"
is_control_irradiator <- meta_final$Dose_condition == "None_irradiator_control"
is_xray_0.05_Gy   <- meta_final$Dose_condition == "0.05_Gy"
is_xray_0.10_Gy   <- meta_final$Dose_condition == "0.10_Gy"
is_xray_0.25_Gy   <- meta_final$Dose_condition == "0.25_Gy"
is_xray_0.50_Gy   <- meta_final$Dose_condition == "0.50_Gy"
is_xray_1.00_Gy   <- meta_final$Dose_condition == "1.00_Gy"
is_xray_2.00_Gy   <- meta_final$Dose_condition == "2.00_Gy"
is_xray_4.00_Gy   <- meta_final$Dose_condition == "4.00_Gy"
is_xray_6.00_Gy   <- meta_final$Dose_condition == "6.00_Gy"


logit(sprintf("is_male=%d is_female=%d 
              is_control_incubator=%d 
              is_control_irradiator =%d 
              is_xray_0.05_Gy =%d 
              is_xray_0.10_Gy =%d 
              is_xray_0.25_Gy =%d
              is_xray_0.50_Gy =%d 
              ...
              sum(is_control_incubator), sum(is_control_irradiator), sum(is_xray_0.05_Gy), sum(is_xray_0.10_Gy), sum(is_xray_0.25_Gy),
              sum(is_xray_0.50_Gy), sum(is_xray_1.00_Gy), sum(is_xray_2.00_Gy), sum(is_xray_4.00_Gy), sum(is_xray_6.00_Gy)
              ))

traits <- data.frame(
  Meta1_Male_only = ifelse(is_male, 1L, ifelse(is_female, 0L, NA)),
  Meta2_Female_only   = ifelse(is_female, 1L, ifelse(is_male, 0L, NA)),
  Meta3_control_incubator_only = ifelse(is_control_incubator, 1L, 0L),
  Meta4_control_irradiator_Only   = ifelse(is_control_irradiator, 1L, 0L),
  Meta5_xray_0.05_Gy_Only  = ifelse(is_xray_0.05_Gy, 1L, 0L),
  Meta6_xray_0.10_Gy_Only  = ifelse(is_xray_0.10_Gy, 1L, 0L),
  Meta7_xray_0.25_Gy_Only  = ifelse(is_xray_0.25_Gy, 1L, 0L),
  Meta8_xray_0.50_Gy_Only  = ifelse(is_xray_0.50_Gy, 1L, 0L),
  Meta9_xray_1.00_Gy_Only  = ifelse(is_xray_1.00_Gy, 1L, 0L),
  Meta10_xray_2.00_Gy_Only  = ifelse(is_xray_2.00_Gy, 1L, 0L),
  Meta11_xray_4.00_Gy_Only  = ifelse(is_xray_4.00_Gy, 1L, 0L),
  Meta12_xray_6.00_Gy_Only  = ifelse(is_xray_6.00_Gy, 1L, 0L),
  stringsAsFactors = FALSE
)
...
write.csv(meta_final, file.path(out_dir, "GSE260589_metadata_aligned.csv"))
saveRDS(traits, file.path(out_dir, "GSE260589_traits.rds"))
write.csv(traits, file.path(out_dir, "GSE260589_traits.csv"))
...
# ---- WGCNA parameters (USER-SET) ----
soft_power       <- 10       # USER DECISION from pickSoftThreshold diagnostics
network_type     <- "signed"
tom_type         <- "signed"
cor_type         <- "bicor"
...
# Save eigengenes
write.csv(MEs, file.path(out_dir, "GSE260589_eigengenes.csv"))
logit("  Saved: GSE260589_eigengenes.csv")
...
ht <- Heatmap(module_trait_cor,
              ...
              column_title = "Module-Trait Relationships (GSE260589- XRAY Irradiation)",
...
trait_cols_ordered <- c("Meta1_Male_only", "Meta2_Female_only", "Meta3_control_incubator_only",
                        "Meta4_control_irradiator_Only", "Meta5_xray_0.05_Gy_Only", "Meta6_xray_0.10_Gy_Only",
                        "Meta7_xray_0.25_Gy_Only", "Meta8_xray_0.50_Gy_Only",
                        "Meta9_xray_1.00_Gy_Only", "Meta10_xray_2.00_Gy_Only", "Meta11_xray_4.00_Gy_Only",
                        "Meta12_xray_6.00_Gy_Only")
...
short_labels <- c("Modules",
                  "Meta1_Male (n=140)", "Meta2_Female (n=109)", "Meta3_control_incubator (n=24)",
                  "Meta4_control_irradiator (n=24)", "Meta5_0.05_Gy (n=26)", "Meta6_0.10_Gy (n=27)",
                  "Meta7_0.25_Gy (n=26)", "Meta8_0.50_Gy (n=22)", "Meta9_1.00_Gy (n=27)", "Meta10_2.00_Gy (n=24)",
                  "Meta11_4.00_Gy (n=26)", "Meta12_6.00_Gy (n=23)"
                  )
...
tmp_xlsx <- "~/GSE260589/GSE260589_WGCNA/wgcna_gene_modules_by_module.xlsx"
