...
anyDuplicated(normcount[, 1])
temp.norm <- normcount 
rownames(temp.norm) <- temp.norm[, 1]
temp.norm <- temp.norm[, -c(1, 40:90)]

# rowGroup <- as.character(rownames(temp.norm))
...
###############################################################################
## Load metadata
GSE290333.meta <- read.csv(meta_path, header = TRUE, sep = ",", check.names = FALSE)
temp.meta <- GSE290333.meta
...
## Clean Sex
sex_clean <- trimws(as.character(temp.meta[["Sex"]]))
...
sex_clean[sex_clean %in% c("N/A", "NA", "")] <- NA_character_

temp.meta[["Sex"]] <- sex_clean


## Clean Age
age_clean <- trimws(as.character(temp.meta[["Age"]]))
...
age_clean[age_clean %in% c("N/A", "NA", "")] <- NA_character_

# Convert remaining values to numeric
temp.meta[["Age"]] <- as.numeric(age_clean)

table(temp.meta$Age)
colnames(temp.meta)
# # temp.meta$'Sample Name' <- sub(" sample$", "_sample", temp.meta$'Sample Name')
# colnames(temp.meta)[colnames(temp.meta) == 'Factor Value[Sex]'] <- 'Sex'
# colnames(temp.meta)[colnames(temp.meta) == 'Factor Value[Ionizing Radiation]'] <- 'Dose_condition'
# colnames(temp.meta)[colnames(temp.meta) == 'Factor Value[Absorbed Radiation Dose]'] <- 'Dose'
# rownames(temp.meta) <- temp.meta$'Sample Name'
temp.meta$Disease_condition <- temp.meta$Sample_source_name_ch1
table(temp.meta$Disease_condition)
temp.meta$Disease_condition <- sub("Control", "HC", temp.meta$Disease_condition)
temp.meta$Disease_condition <- sub("Parkinson’s Disease", "PD", temp.meta$Disease_condition)

rownames(temp.meta) <- temp.meta$Sample_ID
...
get_color_disease_condition <- function(val) {
  if (is.na(val)) return("white")
  if (val == "HC") return("#B2182B")
  if (val == "PD") return("#2166AC")
  return("white")
}

sample_order <- sampleTree$labels[sampleTree$order]
colors_diag  <- sapply(meta[sample_order, "Sex"], get_color_diag)
colors_disease_cond  <- sapply(meta[sample_order, "Disease_condition"], get_color_disease_condition)
...
par(mar = c(7, 5, 3, 2))
plot(sampleTree, main = "Sample Dendrogram (Microarray-Norm) - GSE290333 Outlier Check",
...
plot(0, type = "n", xlim = c(0.5, length(sample_order)), ylim = c(0, 1),
     xaxt = "n", yaxt = "n", xlab = "", ylab = "", bty = "n")
rect(seq(0.5, length(sample_order) - 0.5), 0, seq(1.5, length(sample_order) + 0.5), 1,
     col = colors_disease_cond, border = NA)
mtext("Disease_condition", side = 2, line = 0.5, cex = 0.7, las = 2)
dev.off()
...
saveRDS(datExpr, file.path(out_dir, "GSE290333_datExpr.rds"))
logit(sprintf("  Saved: GSE290333_datExpr.rds (%d samples x %d genes)",
...
is_PD   <- meta_final$Disease_condition == "PD"
is_Control   <- meta_final$Disease_condition == "HC"


logit(sprintf("is_male=%d is_female=%d is_PD=%d is_Control=%d",
              sum(is_male), sum(is_female),
              sum(is_PD), sum(is_Control)
              ))

traits <- data.frame(
  Meta1_Male_only = ifelse(is_male, 1L, ifelse(is_female, 0L, NA)),
  Meta2_Female_only   = ifelse(is_female, 1L, ifelse(is_male, 0L, NA)),
  Meta3_PD_only = ifelse(is_PD, 1L, 0L),
  Meta4_Control_Only   = ifelse(is_Control, 1L, 0L),
  stringsAsFactors = FALSE
)
...
write.csv(meta_final, file.path(out_dir, "GSE290333_metadata_aligned.csv"))
saveRDS(traits, file.path(out_dir, "GSE290333_traits.rds"))
write.csv(traits, file.path(out_dir, "GSE290333_traits.csv"))
...
# Save eigengenes
write.csv(MEs, file.path(out_dir, "GSE290333_eigengenes.csv"))
logit("  Saved: GSE290333_eigengenes.csv")
...
ht <- Heatmap(module_trait_cor,
              ...
              column_title = "Module-Trait Relationships (GSE290333- Gamma Irradiation)",
...
trait_cols_ordered <- c("Meta1_Male_only", "Meta2_Female_only", "Meta3_PD_only",
                        "Meta4_Control_Only")
...
short_labels <- c("Modules",
                  "Meta1_Male (n=20)", "Meta2_Female (n=18)", "Meta3_PD (n=22)",
                  "Meta4_Control_Only (n=16)"
                  )
...
tmp_xlsx <- "~/GSE290333/GSE290333_WGCNA/wgcna_gene_modules_by_module.xlsx"
