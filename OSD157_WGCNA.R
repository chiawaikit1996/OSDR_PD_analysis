...
meta_path   <- "/shared/home/m330498/OSD157/GSE290333_metadata.csv"
out_dir_p2  <- "/shared/home/m330498/OSD157/OSD157_WGCNA"   # Phase 2 outputs
...
normcount <- read.csv("GLDS-157_array_normalized_expression_G4112F_GLmicroarray.csv", header = TRUE, sep = ",", check.names = FALSE)
temp.norm <- normcount %>%
...
###############################################################################
## Load metadata
OSD157.meta <- read.csv(meta_path, header = TRUE, sep = ",", check.names = FALSE)
temp.meta <- OSD157.meta
...
temp.meta$Meta1 <- paste(temp.meta$Sex, temp.meta$Dose, sep = "_")
temp.meta$Meta1 <- sub(" gray$", "_Gy_48Hrs_Gamma", temp.meta$Meta1)
colnames(temp.meta)
table(temp.meta$Meta1)
...
par(mar = c(7, 5, 3, 2))
plot(sampleTree, main = "Sample Dendrogram (Microarray-Norm) - OSD157 Outlier Check",
...
saveRDS(datExpr, file.path(out_dir, "OSD157_datExpr.rds"))
logit(sprintf("  Saved: OSD157_datExpr.rds (%d samples x %d genes)",
...
is_sham_0_Gy   <- meta_final$Dose == "0 gray"
is_gamma_0.5_Gy   <- meta_final$Dose == "0.5 gray"
is_gamma_2_Gy   <- meta_final$Dose == "2 gray"
is_gamma_5_Gy   <- meta_final$Dose == "5 gray"
is_gamma_8_Gy   <- meta_final$Dose == "8 gray"

logit(sprintf("is_male=%d is_female=%d is_sham_0_Gy=%d is_gamma_0.5_Gy =%d is_gamma_2_Gy =%d is_gamma_5_Gy =%d is_gamma_8_Gy =%d",
...
write.csv(meta_final, file.path(out_dir, "OSD157_metadata_aligned.csv"))
saveRDS(traits, file.path(out_dir, "OSD157_traits.rds"))
write.csv(traits, file.path(out_dir, "OSD157_traits.csv"))
...
# ---- WGCNA parameters (USER-SET) ----
soft_power       <- 26       # USER DECISION from pickSoftThreshold diagnostics
network_type     <- "signed"
tom_type         <- "signed"
cor_type         <- "bicor"
...
# Save eigengenes
write.csv(MEs, file.path(out_dir, "OSD157_eigengenes.csv"))
logit("  Saved: OSD157_eigengenes.csv")
...
ht <- Heatmap(module_trait_cor,
              ...
              column_title = "Module-Trait Relationships (OSD157- Gamma Irradiation)",
...
short_labels <- c("Modules",
                  "Meta1_Male (n=10)", "Meta2_Female (n=15)", "Meta3_Sham (n=5)",
                  "Meta4_0.5_Gy (n=5)", "Meta5_2_Gy (n=5)", "Meta6_8_Gy (n=5)"
                  )
...
tmp_xlsx <- "~/OSD157/OSD157_WGCNA/wgcna_gene_modules_by_module.xlsx"
